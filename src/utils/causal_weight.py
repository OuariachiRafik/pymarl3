...
def get_s2s_weight(env, memory, agent, sample_size=5000, causal_method='DirectLiNGAM'):
    states, actions, rewards, next_states, dones = memory.sample(sample_size)
...
def mask_irrelevant_states(states, weight_state_to_state, top_k=80):
    """
    ...
    # 确保权重是 numpy 数组
    weight_state_to_state = np.array(weight_state_to_state)
    ...
    top_indices = np.argsort(weight_state_to_state)[-top_k:]  # 选取 top_k 个最大的索引
...
def mask_irrelevant_actions(actions, weight_action_to_state, top_k=80):
    """
    使用 state-reward 因果矩阵对状态维度进行筛选，mask 掉最不相关的维度
    ...
    # 确保权重是 numpy 数组
    weight_action_to_state = np.array(weight_action_to_state)
    ...
    top_indices = np.argsort(weight_action_to_state)[-top_k:]  # 选取 top_k 个最大的索引
    mask = np.zeros(state_dim)  # 初始化为全 0
    mask[top_indices] = 1  # 仅保留前 top_k 维度

    # **应用 mask**
    masked_actions = actions * mask  # 按位乘法，屏蔽无关维度
