...
class StateSemanticEncoder(nn.Module):
    ...
    """
    def __init__(self, ally_dim, enemy_dim, n_allies, n_enemies,
                 action_dim=0, out_dim=128):
        super().__init__()
        self.n_allies, self.n_enemies = n_allies, n_enemies
        self.ally_dim = ally_dim + (action_dim if action_dim>0 else 0)
    ...
    def forward(self, ally_feats, enemy_feats,
                ally_mask=None, enemy_mask=None,
        ...
        a = self.ally_enc(ally_feats, ally_mask)      # [B,T,out/2]
        e = self.enemy_enc(enemy_feats, enemy_mask)   # [B,T,out/2]
