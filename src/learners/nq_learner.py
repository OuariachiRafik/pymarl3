...
def calculate_n_step_td_target(target_mixer, target_max_qvals, states_for_mixer, rewards, terminated, mask, gamma,
                               td_lambda,
...
class NQLearner:
    def __init__(self, mac, scheme, logger, args, state_layout):
        ...
        if self.use_cmi_mask:
            ...
            print("######################################################## act_dim = ", act_dim, "n_actions = ",
                  args.n_actions)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, causal_update):
        ...
        if self.use_state_blocks and getattr(self.args, "state_blocks_transition_pretrain", True) and causal_update > 8000:
            B, T, _ = z_t.shape
        ...
        # ---- UPDATE CMI MASKER -----------------------------------------
        if self.use_cmi_mask and causal_update > 8000:  # and causal_update > 2000 and causal_update % 1000==0 :
            ...
            cmi_logs = self.cmi_masker.step_train_minibatch(Z_flat[sample_indices], A_flat[sample_indices],
        ...
        if self.use_state_blocks and causal_update > 8000:
            if self.use_cmi_mask:
                ...
                if os.path.exists(save_path):
                    ...
                    new_data = np.concatenate([old_data, [M_cpu]], axis=0)
                else:
                    # 第一次保存
                    new_data = np.array([M_cpu])
